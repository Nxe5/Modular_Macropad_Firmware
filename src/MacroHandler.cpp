...
bool MacroHandler::begin() {
    // LittleFS should already be initialized in main.cpp, so we just check if it's mounted
    if (!LittleFS.exists("/")) {
        USBSerial.println("LittleFS is not mounted properly for macro storage");
        return false;
    }
